...
export default function FillingPage() {
  const serviceDetail = serviceDetails["filling"];
                ...
                <li key={index} className="text-xs sm:text-sm font-medium">
                  ...
                  <ul className="list-disc ml-4 mt-1">
                    {benefit.details.map((detail, detailIndex) => (
                      <li key={detailIndex} className="font-normal text-xs">
                        {detail}
                      </li>
                    ))}
                  </ul>
        ...
        {serviceDetail.types && (
          <div className="my-3">
            ...
            {serviceDetail.types.items.map((type, index) => (
              <div
                key={index}
                className={`border ${type.borderColor === "pink"
                  ? "border-pink-300"
                  : "border-gray-300"
                  } rounded p-2 sm:p-3 ${index < serviceDetail.types!.items.length - 1 ? "mb-3" : ""
                  }`}
              >
                <h3 className="text-xs sm:text-sm font-bold">{type.title}</h3>
                <p className="text-xs my-1">
                  <span className="font-medium">{uiText.characteristics}</span>{" "}
                  {type.description}
                </p>
                <p className="text-xs my-1">
                  <span className="font-medium">{uiText.advantages}</span>
                </p>
                ...
                {type.disadvantages && type.disadvantages.length > 0 && (
                  <>
                    <p className="text-xs my-1">
                      <span className="font-medium">{uiText.disadvantages}:</span>
                    </p>
                    <ul className="list-disc pl-5 text-xs">
                      {type.disadvantages.map((disadvantage, disIndex) => (
                ...
                {type.suitableFor && (
                  <p className="text-xs mt-1">
                    <span className="font-medium">{uiText.suitableFor}</span>{" "}
                    {type.suitableFor}
                  </p>
                )}
                {type.duration && (
                  <p className="text-xs mt-1">
                    <span className="font-medium">{uiText.duration}</span>{" "}
                    {type.duration}
                  </p>
                )}
              </div>
